...
{  
    ...
    public Mode EXPMode;
    public enum TriggerTime {Single, Multiple};
    public TriggerTime TriggerMode;
    ...
    void Start()
    {
        ...
        switch(TriggerMode){
			case TriggerTime.Single:
				Single = true;
				break;
			case TriggerTime.Multiple:
				Multiple = true;
				break;
		}
    }
    ...
    public void doGazeStuff(bool isStart = true){
        // mRender.enabled = true;
        // coli.enabled = false;
        if(Multiple &&　isStart && this.identifyLabel() != 0 && lighting){
            lightcoroutine = changeMaterial();
            if(lightcoroutine != null)
                StopCoroutine(lightcoroutine);
            StartCoroutine(lightcoroutine);
        }
        if(Multiple && isStart&& this.identifyLabel() != 0 && shake && shakeflag){
            shakeflag = false;
            Shake(1, 0.5f, delaytime);
        }
        if(Multiple && this.identifyLabel() != 0 && sound){
        ...
        //Single
        if(Single &&　isStart && this.identifyLabel() != 0 && lighting){
            coli.enabled = false;
            lightcoroutine = changeMaterial_Single();
            if(lightcoroutine != null)
                StopCoroutine(lightcoroutine);
            StartCoroutine(lightcoroutine);
        }
        if(Single && isStart&& this.identifyLabel() != 0 && shake && shakeflag){
            coli.enabled = false;
            shakeflag = false;
            Shake_Single(1, 0.5f, delaytime);
        }
        if(Single && this.identifyLabel() != 0 && sound){
            coli.enabled = false;
            soundcoroutine = changeSound_Single(delaytime);
            if(soundcoroutine != null)
                StopCoroutine(soundcoroutine);
            StartCoroutine(soundcoroutine);
        }
    ...
    private IEnumerator changeMaterial_Single()
    {
        Debug.Log("before delay");
        yield return new WaitForSeconds(delaytime);
        Debug.Log("After Delay");
    ...
    private IEnumerator changeSound_Single (float delayTime)
    {
        yield return new WaitForSeconds(delayTime);
        this.GetComponent<AudioSource>().PlayOneShot(this.GetComponent<AudioSource>().clip, 0.5f);
    }
    ...
    public void Shake_Single (float duration, float amount, float delayTime) {
        _originalPos = transform.localPosition;
        shakecoroutine = cShake_Single(duration, amount, delayTime);
        if(shakecoroutine != null)
            StopCoroutine(shakecoroutine);
        StartCoroutine(shakecoroutine);
    }
    public IEnumerator cShake (float duration, float amount, float delayTime) {
      ...
      public IEnumerator cShake_Single (float duration, float amount, float delayTime) {
        if(!shakeflag)
        {
            coli.enabled = false;
            yield return new WaitForSeconds(delayTime);
